...
pairing.ensureSessionDir(sessionId)
const {  state, saveCreds } = await useMultiFileAuthState(pairing.sessionDir(sessionId))
const store = createStore()
let connectedMessageSent = false
const __timers = []
const __setInterval = (fn, ms) => { const t = setInterval(fn, ms); __timers.push(t); return t }
   ...
   registerWASocket(sessionId, EliteProTech)
   pairing.register(sessionId, EliteProTech, {
       phone: opts.pairPhone || null,
       tgChatId: opts.tgChatId || null,
       tgUserId: opts.tgUserId || null,
       source: opts.source || 'telegram',
   })
   EliteProTech.__timers = __timers
   EliteProTech.__sessionId = sessionId
   store.bind(EliteProTech.ev)


   if (opts.pairPhone && !EliteProTech.authState.creds.registered) {
      // NOTE: this baileys fork does NOT export PHONENUMBER_MCC, so the old
      // country-code check is replaced by a simple sanity check. Reading the
      // missing export used to throw and the pairing code was never requested.
      const pairPhoneClean = String(opts.pairPhone).replace(/[^0-9]/g, '')
      if (!pairPhoneClean || pairPhoneClean.length < 7) {
         console.log(chalk.bgBlack(chalk.redBright(`[PAIR ${sessionId}] invalid number: ${pairPhoneClean} (use country code, e.g. 2347047504860)`)))
      }

      const requestCode = async (attempt) => {
         try {
            let code = await EliteProTech.requestPairingCode(pairPhoneClean)
            const pretty = code?.match(/.{1,4}/g)?.join("-") || code
            console.log(chalk.black(chalk.bgGreen(`[PAIR ${sessionId}] Code : `)), chalk.black(chalk.white(pretty)))

            // Website pairing: hand the raw code back so the site can display it.
            if (typeof opts.onPairingCode === 'function') {
               try { await opts.onPairingCode(code, pretty) } catch (e) { console.log('[PAIR] callback failed:', e.message) }
            }

            // Telegram pairing: deliver the code to the chat that asked for it.
            if (opts.tgChatId && mainBot) {
               mainBot.telegram.sendMessage(
                  opts.tgChatId,
                  `🔑 *Pairing code for +${pairPhoneClean}*\n\n\`${pretty}\`\n\nOpen WhatsApp → Linked Devices → Link a device → Link with phone number`,
                  { parse_mode: 'Markdown' }
               ).catch(() => {})
            }
         } catch (e) {
            console.log(chalk.bgBlack(chalk.redBright(`[PAIR ${sessionId}] attempt ${attempt}/3 failed: ${e.message}`)))
            if (attempt < 3) return setTimeout(() => requestCode(attempt + 1), 5000)

            if (typeof EliteProTech.__pairingError === 'function') {
               try { await EliteProTech.__pairingError(`WhatsApp pairing code request failed: ${e.message}`) } catch (_) {}
            }
            if (opts.tgChatId && mainBot) {
               mainBot.telegram.sendMessage(opts.tgChatId, `❌ Pairing failed: ${e.message}`).catch(() => {})
            }
         }
      }
...
EliteProTech.ev.on("connection.update", async (s) => {
 ...
 if (
     ...
     if (statusCode === 401 || statusCode === DisconnectReason.loggedOut) {
         console.log(chalk.red(`[SESSION ${sessionId}] device logged out - session removed, re-pair required.`));
         pairing.unregister(sessionId);
         pairing.markDisconnected(sessionId);
         try { pairing.deleteSessionDir(sessionId) } catch {}
         if (opts.tgChatId && mainBot) {
    ...
    // ── pairing layer bookkeeping (kept separate from the bot's own handler) ──
    EliteProTech.ev.on('connection.update', async ({ connection }) => {
        try {
            if (connection === 'open') {
                const waNum = String(EliteProTech.user?.id || '').split(':')[0].split('@')[0].replace(/\D/g, '')
                pairing.markConnected(sessionId, waNum)
                console.log(chalk.green(`[SESSION ${sessionId}] connected +${waNum} (${pairing.activeCount()} live)`))
                if (opts.tgChatId && mainBot && !opts.__connectNotified) {
                    opts.__connectNotified = true
                    mainBot.telegram.sendMessage(opts.tgChatId, `✅ *Connected!*\nNumber: wa.me/${waNum}`, { parse_mode: 'Markdown' }).catch(() => {})
                }
            }
        } catch (_) { /* ignore */ }
    })
...
const sessionDir = pairing.sessionDir(sessionId);
...
__setInterval(cleanSessionFiles, 8 * 60 * 60 * 1000);
////Sticker code    
    ...
    // Hand the live socket back to the caller (website bridge, Telegram bot, tooling).
    return EliteProTech
}
...
//   /pair  /listpaired  /delpair  /refreshsession
//   Every paired number runs the full ELITE-PRO bot.
// ============================================================

const BTN_PAIR    = '🔗 PAIR NUMBER'
const BTN_LIST    = '📋 MY NUMBERS'
const BTN_REFRESH = '🔄 REFRESH SESSIONS'
const BTN_DELETE  = '🗑️ DELETE NUMBER'

const PAIR_HELP =
  '🔐 *' + (global.botname || 'ELITE-PRO-V1') + ' – Pairing*\n\n' +
  'Pair here or on the website — every paired number runs the full bot.\n\n' +
  '━━━ 📱 *Commands* ━━━\n' +
  '`/pair <number>` – pair a number (example `/pair 2347047504860`)\n' +
  '`/listpaired` – list your paired numbers\n' +
  '`/refreshsession` – restart your sessions\n' +
  '`/delpair <number>` – disconnect & delete a number\n' +
  '`/cancel` – cancel the current step\n\n' +
  '━━━ 🌐 *Website* ━━━\n' +
  'Website pairing requests are picked up automatically — no command needed.'
...
function pairingKeyboard() {
  return {
    reply_markup: {
      keyboard: [
        [{ text: BTN_PAIR }],
        [{ text: BTN_LIST }, { text: BTN_REFRESH }],
        [{ text: BTN_DELETE }],
      ],
...
// ── Pair one number (Telegram) ───────────────────────────────
async function pairNumber(ctx, phone, uid) {
  const sessionId = `tg_${uid}_${phone}`;

  const ownerInfo = pairing.ownerOf(sessionId);
  if (ownerInfo && String(ownerInfo.tgUserId) !== String(uid)) {
    return ctx.reply('⛔ That number is already paired by another user.');
  }

  const live = pairing.get(sessionId);
  if (live && live.status === 'connected') {
    return ctx.reply(`✅ +${phone} is already connected.`);
  }

  pairing.addPair(uid, sessionId, phone);

  if (pairing.isLive(sessionId)) {
    await ctx.reply(`♻️ Restarting +${phone}...`);
  } else {
    await ctx.reply(`🔄 Pairing *+${phone}*...`, { parse_mode: 'Markdown' });
  }

  try {
    // startSession requests a code only when the creds are not registered yet,
    // so an already-paired number simply reconnects.
    ...
    await ctx.reply(`❌ Failed to start pairing: ${e.message}`);
  }
}

// ── Delete one number (Telegram) ─────────────────────────────
async function deletePair(ctx, phone, uid) {
  const sessionId = `tg_${uid}_${phone}`;

  const ownerInfo = pairing.ownerOf(sessionId);
  if (ownerInfo && String(ownerInfo.tgUserId) !== String(uid)) {
    return ctx.reply('⛔ That number is paired by another user.');
  }

  const wasLive = pairing.isLive(sessionId);
  await pairing.stopSession(sessionId);
  pairing.removePair(uid, sessionId);
  const deleted = pairing.deleteSessionDir(sessionId);

  await ctx.reply((wasLive || deleted)
    ? `🗑️ +${phone} disconnected and deleted. Use /pair ${phone} to connect again.`
...
function setupTelegramBot() {
  ...
  bot.start(async (ctx) => {
    await ctx.reply(PAIR_HELP, { parse_mode: 'Markdown', ...pairingKeyboard() });
    await ctx.reply('Tap below or type /pair 2347047504860 to start:', {
      ...Markup.inlineKeyboard([[Markup.button.callback('📱 Pair a number', 'pair_new')]]),
    }).catch(() => {});
  });
  ...
  bot.action('pair_new', async (ctx) => {
    await ctx.answerCbQuery().catch(() => {});
    states.set(String(ctx.from.id), 'pair');
    await ctx.reply('📱 Send the number to pair, e.g. `2347047504860`', { parse_mode: 'Markdown' });
  });

  bot.command('pair', async (ctx) => {
    const phone = (ctx.message.text.split(/\s+/)[1] || '').replace(/\D/g, '');
    if (!phone || phone.length < 7) {
      return ctx.reply('Usage: `/pair 2347047504860`', { parse_mode: 'Markdown' });
    }
    await pairNumber(ctx, phone, String(ctx.from.id));
  });

  bot.command('listpaired', async (ctx) => {
    const uid = String(ctx.from.id);
    const pairs = pairing.getUserPairs(uid);
    if (!pairs.length) return ctx.reply('No paired numbers yet. Use /pair 2347047504860');
    const lines = pairs.map((p, i) => {
      const e = pairing.get(p.sessionId);
      const dot = e && e.status === 'connected' ? '🟢' : e ? '🟡' : '🔴';
      return `${i + 1}. +${p.phone || '?'} — ${dot} ${p.sessionId}`;
    }).join('\n');
  ...
  bot.command('delpair', async (ctx) => {
    const phone = (ctx.message.text.split(/\s+/)[1] || '').replace(/\D/g, '');
    if (!phone) return ctx.reply('Usage: /delpair 2347047504860');
    await deletePair(ctx, phone, String(ctx.from.id));
  });

  bot.command('refreshsession', async (ctx) => {
    const uid = String(ctx.from.id);
    const pairs = pairing.getUserPairs(uid);
    if (!pairs.length) return ctx.reply('No paired numbers to refresh.');

    await ctx.reply(`🔄 Restarting ${pairs.length} session(s)...`);
    let done = 0;
    for (const p of pairs) {
      try {
        await pairing.stopSession(p.sessionId);
        await startSession(p.sessionId, { source: 'telegram', tgChatId: ctx.chat.id, tgUserId: uid });
        done++;
      } catch (e) {
        console.log(`[SESSION ${p.sessionId}] refresh failed:`, e.message);
      }
    }
    await ctx.reply(`✅ Restarted ${done}/${pairs.length} session(s).`);
  });
  ...
  bot.command('status', async (ctx) => {
    if (OWNER_TELEGRAM_ID && String(ctx.from.id) !== String(OWNER_TELEGRAM_ID)) {
      return ctx.reply('⛔ Owner only.');
    }
    const list = pairing.statusList();
    const text = list.length
      ? list.map(s => `${s.status === 'connected' ? '🟢' : '🟡'} ${s.id} (+${s.waNum || '?'}) [${s.source}]`).join('\n')
      : 'No sessions running.';
    await ctx.reply(`🖥 *Live sessions (${list.length}):*\n\n${text}`, { parse_mode: 'Markdown' });
  });
  ...
  bot.on('message', async (ctx, next) => {
    ...
    if (text === BTN_PAIR) {
      states.set(uid, 'pair');
      return ctx.reply('📱 Send the number to pair, e.g. `2347047504860`', { parse_mode: 'Markdown' });
    }
    if (text === BTN_LIST)    return ctx.telegram.sendMessage(ctx.chat.id, '/listpaired');
    if (text === BTN_REFRESH) return ctx.telegram.sendMessage(ctx.chat.id, '/refreshsession');
    if (text === BTN_DELETE) {
      states.set(uid, 'delpair');
      return ctx.reply('🗑️ Send the number to delete, e.g. `2347047504860`', { parse_mode: 'Markdown' });
    }
    ...
    if (state) {
      ...
      if (!phone || phone.length < 7) {
        return ctx.reply('❌ Invalid number. Example: 2347047504860');
      }
      states.delete(uid);
      if (state === 'pair')    return pairNumber(ctx, phone, uid);
      if (state === 'delpair') return deletePair(ctx, phone, uid);
    }
  ...
  bot.launch({ dropPendingUpdates: true })
    .then(() => console.log(chalk.green('[TELEGRAM] pairing bot running')))
    .catch(e => console.log(chalk.red('[TELEGRAM] launch failed: ' + e.message)));
...
async function launch() {
  ...
  // 1) The owner's own session from SESSION_ID / ./session keeps working.
  if (pairing.sessionExists(pairing.LEGACY_ID)) {
    console.log(chalk.cyan('[BOOT] starting owner session (./session)'));
    startSession(pairing.LEGACY_ID, { source: 'legacy' })
      .catch(e => console.log(`[SESSION main] failed: ${e.message}`));
  }

  // 2) Reload every previously paired session.
  const saved = pairing.listSessions().filter(id => id !== pairing.LEGACY_ID);
  console.log(chalk.cyan(`[BOOT] reloading ${saved.length} paired session(s)`));
  saved.forEach((sid, i) => {
    const meta = pairing.ownerOf(sid);
    setTimeout(() => {
      startSession(sid, {
        source: sid.startsWith('web_') ? 'website' : 'telegram',
        tgChatId: (meta && meta.tgChatId) || null,
        tgUserId: (meta && meta.tgUserId) || null,
      }).catch(e => console.log(`[SESSION ${sid}] start failed: ${e.message}`));
    }, i * 1500);
  });

  // 3) Website pairing bridge (Neon: varnox_pairing_requests).
  startWebsitePairingLoop((sessionId, phone, codeCb) =>
    startSession(sessionId, {
      pairPhone: phone,
      source: 'website',
      onPairingCode: async (code) => { await codeCb(code); },
    }).catch(e => console.log('[WEBSITE-PAIR] start failed: ' + e.message))
  );
...
launch().catch(e => console.log('LAUNCH FAILED:', e && e.message));

// ── exported so tooling/tests can drive a session directly ────
module.exports = { startSession, pairing, launch };
